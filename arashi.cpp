...
void playerPrint();
void removePlayer();
void playerJump(int,int,int);
void playerAttack();
void playerRun();

int pX = 50, pY = 28; // player coordinates

int camX = 0, camY = 0; // camera coordinates

string activeScreen, gameState, playerState = "idle", playerDir="right"; // player states
...
void gameScreen(){
    ...
    while(1){
        ...
        // movements & idle
        if((GetAsyncKeyState(VK_RIGHT) || GetAsyncKeyState('D')) && pX <= 104){
            playerDir = "right";
            playerRun();
        }
        else if((GetAsyncKeyState(VK_LEFT) || GetAsyncKeyState('A')) && pX >= 3){
            playerDir = "left";
            playerRun();
        }
        else{
...
void playerJump(int force, int gravity, int discplacement){
    if(playerState == "run"){
        if(playerDir == "left" && pX > 15)
            discplacement = -10;
        else if(playerDir == "right" && pX < 89)
            discplacement = 10;
    }
